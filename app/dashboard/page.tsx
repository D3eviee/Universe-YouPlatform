  ...
  return (
    <div className="w-full h-[calc(100vh-55px)] flex flex-row">
      ...
      <main className="w-full flex flex-row gap-3 border-l-[0.5px] border-l-black  bg-primary-dark p-5 resize-none">
        <section className=" h-full flex flex-col mx-2 overflow-hidden">
          <QuoteToolbar/>
            ...
            <div className="w-full flex flex-col gap-5">
              {activeArticle?.blocks.map((block: EditorBlock) => {
                switch (block.type) {
                  ...
                  case "heading":
                    return <HeadingInput key={block.id} id={block.id}  value={block.data} onChange={(newValue) => updateBlockData(block.id, newValue)}/>
                  case "image":
